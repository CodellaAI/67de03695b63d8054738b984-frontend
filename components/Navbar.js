...
import { Menu, Search, Bell, Upload, User, LogOut, X, ChevronDown } from 'lucide-react'
...
export default function Navbar() {
  ...
  return (
    <header className="bg-yt-black sticky top-0 z-50">
      <div className="flex items-center justify-between h-16 px-4">
        {/* Logo and menu button */}
        <div className="flex items-center">
          <button 
            className="p-2 rounded-full hover:bg-gray-800 mr-2 lg:mr-4"
        ...
        {/* Search bar - desktop */}
        <div className={`hidden md:flex flex-1 max-w-xl mx-4 ${showMobileSearch ? 'hidden' : ''}`}>
          <form onSubmit={handleSearch} className="flex w-full">
            <input
              type="text"
              placeholder="Search"
              className="w-full bg-yt-black border border-gray-700 rounded-l-full px-4 py-2 focus:outline-none focus:border-blue-500"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
            />
            <button
              type="submit"
              className="bg-gray-800 border border-gray-700 rounded-r-full px-6 hover:bg-gray-700"
              aria-label="Search"
            >
              <Search size={20} />
            </button>
        ...
        {/* Search bar - mobile */}
        {showMobileSearch && (
          <div className="fixed inset-0 bg-yt-black z-50 px-4 flex items-center">
            <form onSubmit={handleSearch} className="flex w-full">
              <button
                type="button"
                className="mr-4"
        ...
        <div className="flex items-center">
          ...
          {user ? (
            <>
              <Link href="/upload" className="p-2 rounded-full hover:bg-gray-800 ml-2">
                <Upload size={24} />
              </Link>
              
              <button className="p-2 rounded-full hover:bg-gray-800 ml-2">
                <Bell size={24} />
              </button>
              
              <DropdownMenu.Root>
                <DropdownMenu.Trigger asChild>
                  <button className="ml-4 rounded-full overflow-hidden">
                    <Avatar.Root className="w-8 h-8 rounded-full">
                      ...
                      />
                      <Avatar.Fallback className="bg-gray-700 flex items-center justify-center text-white font-medium">
                ...
                <DropdownMenu.Portal>
                  <DropdownMenu.Content
                    className="min-w-[220px] bg-gray-800 rounded-md p-2 shadow-lg"
                    sideOffset={5}
                    align="end"
                  >
                    ...
                    <DropdownMenu.Item className="flex items-center px-3 py-2 text-sm rounded hover:bg-gray-700 cursor-pointer">
                      <Link href={`/channel/${user._id}`} className="flex items-center w-full">
                    ...
                    <DropdownMenu.Item 
                      className="flex items-center px-3 py-2 text-sm rounded hover:bg-gray-700 cursor-pointer text-red-500"
                      onClick={logout}
                    >
            ...
            <Link 
              href="/login" 
              className="flex items-center border border-blue-600 text-blue-600 px-3 py-1.5 rounded-full text-sm font-medium hover:bg-blue-600/10"
            >
